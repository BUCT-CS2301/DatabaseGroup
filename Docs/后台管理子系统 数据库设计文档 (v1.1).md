# 后台管理子系统 数据库设计文档 (v1.1)

## 文档版本修订

| 版本   | 日期       | 修订说明 |
| :----- | :--------- | :------- |
| v1.1   | 2026-06-01 | 新增5张表：artifact_view_log（文物浏览记录）、user_favorite（用户收藏）、user_browse_history（用户浏览历史）、comment_like（评论点赞）、artifact_like（文物点赞）。 |
| v1.0   | —          | 初版。   |
...
- **全局唯一标识**：所有表主键统一使用 `object_id`（字符串唯一标识），对外暴露的接口一律使用该字段，符合接口文档规范。
- **全平台统一用户表**：将后台管理员、知识服务子系统用户、掌上博物馆用户纳入同一张 `user` 表；当前后台管理场景中，`user_type` 同时承担用户所属角色编码，与 `role.role_code` 对应。
- **RBAC 权限模型**：当前后台采用轻量 RBAC 实现，`user.user_type` 保存用户所属角色编码并对应 `role.role_code`，`role.permissions` 以逗号分隔保存权限标识，支持角色权限动态配置。
...
- 当前后台用户通过 `user.user_type` 绑定单个角色编码，角色编码对应 `role.role_code`。
- 一个角色可分配给多个用户，角色下用户数通过 `user.user_type = role.role_code` 统计。
- 角色权限直接存储在 `role.permissions` 字段中，多个权限标识用英文逗号分隔；权限目录由后端接口维护，不单独落独立权限表。
- 文物归属博物馆，博物馆与文物为一对多。
...
| 字段名          | 类型                                       | 约束                               | 说明                                               |
| :-------------- | :----------------------------------------- | :--------------------------------- | :------------------------------------------------- |
| object_id       | VARCHAR(64)                                | PK                                 | 全局唯一标识，对应后端 `ASSIGN_UUID` 主键策略      |
| username        | VARCHAR(100)                               | UNIQUE, NOT NULL                   | 登录用户名                                         |
| password_hash   | VARCHAR(255)                               | NOT NULL                           | 加密后的密码（BCrypt）                             |
| nickname        | VARCHAR(100)                               | DEFAULT ''                         | 昵称/真实姓名                                      |
| email           | VARCHAR(200)                               | DEFAULT ''                         | 邮箱                                               |
| phone           | VARCHAR(20)                                |                                    | 手机号                                             |
| avatar          | VARCHAR(500)                               |                                    | 头像URL                                            |
| user_type       | VARCHAR(50)                                | DEFAULT 'ADMIN'                    | 当前后台中作为角色编码使用，对应 `role.role_code`；后端新增用户未传角色时默认写入 `USER` |
| status          | VARCHAR(20)                                | DEFAULT 'ENABLED'                  | 账号状态                                           |
...
| 字段名      | 类型         | 约束             | 说明                                                       |
| :---------- | :----------- | :--------------- | :--------------------------------------------------------- |
| object_id   | VARCHAR(64)  | PK               | 角色唯一标识                                               |
| role_name   | VARCHAR(100) | NOT NULL         | 角色名称（如 Super Admin、Content Auditor）                |
| role_code   | VARCHAR(50)  | UNIQUE, NOT NULL | 角色编码（如 ADMIN、AUDITOR、USER），对应 `user.user_type` |
| description | VARCHAR(500) | DEFAULT ''       | 描述                                                       |
| permissions | TEXT         |                  | 权限标识列表，多个权限用英文逗号分隔                       |
...
当前权限数据不再单独创建 `permission`、`user_role`、`role_permission` 三张表。角色与用户的对应关系由 `user.user_type = role.role_code` 表达；角色拥有的权限由 `role.permissions` 保存，示例：

```text
dashboard:view,user:read,user:write,role:read,role:write,audit:read,audit:write,data:read,data:write,backup:read,backup:write,log:read,settings:read,settings:write
```
...
| object_id    | role_name       | role_code | permissions 摘要                                                                                             | is_system |
| :----------- | :-------------- | :-------- | :------------------------------------------------------------------------------------------------------------ | :-------- |
| role-admin   | Super Admin     | ADMIN     | 包含 dashboard、user、role、audit、data、backup、log、settings 的读写权限                                     | 1         |
| role-auditor | Content Auditor | AUDITOR   | `audit:read,audit:write`                                                                                      | 1         |
| role-user    | Normal User     | USER      | `profile:read`（普通用户个人资料读取权限，当前后台权限目录中预留）                                             | 1         |
...
#### 表 `artifact_view_log`（文物浏览记录表）

| 字段名      | 类型        | 约束                               | 说明                                               |
| :---------- | :---------- | :--------------------------------- | :------------------------------------------------- |
| object_id   | VARCHAR(36) | PK                                 | 全局唯一标识                                       |
| artifact_id | VARCHAR(36) | NOT NULL, FK -> artifact.object_id | 关联文物ID                                         |
| user_id     | VARCHAR(36) | FK -> user.object_id               | 浏览用户ID（可为空，表示匿名访问）                 |
| view_time   | DATETIME    | NOT NULL DEFAULT CURRENT_TIMESTAMP | 浏览时间                                           |
| ip_address  | VARCHAR(45) |                                    | 访问者IP地址                                       |

**索引**：
- `KEY idx_artifact_views (artifact_id, view_time)`
...
| :----------- | :----------- | :--------------------------------- | :------------------------------------------------- |
| object_id    | VARCHAR(36)  | PK                                 | 全局唯一标识                                       |
| user_id      | VARCHAR(36)  | NOT NULL, FK -> user.object_id     | 用户ID                                             |
| artifact_id  | VARCHAR(36)  | NOT NULL, FK -> artifact.object_id | 收藏的文物ID                                       |
| group_name   | VARCHAR(100) | DEFAULT 'default'                  | 收藏分组名称                                       |
...
| object_id     | VARCHAR(36) | PK                                 | 全局唯一标识                                       |
| user_id       | VARCHAR(36) | NOT NULL, FK -> user.object_id     | 用户ID                                             |
| artifact_id   | VARCHAR(36) | NOT NULL, FK -> artifact.object_id | 浏览的文物ID                                       |
| browse_time   | DATETIME    | NOT NULL DEFAULT CURRENT_TIMESTAMP | 浏览时间                                           |

**索引**：
- `KEY idx_user_browse (user_id, browse_time DESC)`
...
| :----------- | :---------- | :--------------------------------- | :------------------------------------------------- |
| object_id    | VARCHAR(36) | PK                                 | 全局唯一标识                                       |
| comment_id   | VARCHAR(36) | NOT NULL, FK -> ugc_comment.object_id | 评论ID                                         |
| user_id      | VARCHAR(36) | NOT NULL, FK -> user.object_id     | 点赞用户ID                                         |
| create_time  | DATETIME    | NOT NULL DEFAULT CURRENT_TIMESTAMP | 点赞时间                                           |

**索引**：
- `UNIQUE KEY uk_comment_user (comment_id, user_id)`

#### 表 `artifact_like`（文物点赞表）

| 字段名       | 类型        | 约束                               | 说明                                               |
| :----------- | :---------- | :--------------------------------- | :------------------------------------------------- |
| object_id    | VARCHAR(36) | PK                                 | 全局唯一标识                                       |
| artifact_id  | VARCHAR(36) | NOT NULL, FK -> artifact.object_id | 文物ID                                             |
| user_id      | VARCHAR(36) | NOT NULL, FK -> user.object_id     | 点赞用户ID                                         |
| create_time  | DATETIME    | NOT NULL DEFAULT CURRENT_TIMESTAMP | 点赞时间                                           |
...
| 字段                     | 可选值                                |
| :----------------------- | :------------------------------------ |
| user_type                | 当前后台默认 ADMIN、AUDITOR、USER；可扩展为其他 `role.role_code` |
...
- **权限初始化**：部署时通过脚本初始化至少三条内置角色（`ADMIN`、`AUDITOR`、`USER`）；权限标识以逗号分隔写入 `role.permissions`，权限目录由后端 `/roles/permissions` 接口返回。
