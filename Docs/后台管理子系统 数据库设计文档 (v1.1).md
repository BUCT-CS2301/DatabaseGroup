...
| 版本 | 日期 | 修订说明 |
| :--- | :--- | :--- |
| v1.1.1 | 2026-06-02 | **互动表补丁**：新增幂等脚本 `docker/mysql/04-interaction-tables-hotfix.sql`，兜底创建 `comment_like`、`artifact_like`、`user_favorite`、`user_browse_history`，修复历史环境缺表导致的接口 SQL 异常。 |
| v1.1 | 2026-06-01 | **文物多图存储**：`artifact` 表删除 **`image_url`**、**`image_path`**；新增 **`artifact_image`**（`file_name` PK + `artifact_id` FK）；补充与 API/落盘、静态访问的说明。 |
| v1.0 | — | 初版。 |
...
erDiagram
    ...
    ARTIFACT ||--o{ ARTIFACT_IMAGE : has_images
...
- 文物归属博物馆，博物馆与文物为一对多。
- 文物图片元数据存 **`artifact_image`**：一件文物对应多行，主键为落盘文件名 **`file_name`**（UUID v7 + 扩展名）；物理文件存 **`relics-images/`**，对外 URL 由应用配置 **`app.relics.image-public-base-url`** 拼接，**不**在 `artifact` 表冗余图片字段。
...
> **图片数据**：业务图片不再写入本表。上传接口每次向 **`artifact_image`** 插入一行；接口层按 `artifact_id` 查询全部 `file_name`（升序）生成 `imageUrl` / `imageUrls`。

#### 表 `artifact_image` (文物图片索引)

| 字段名 | 类型 | 约束 | 说明 |
| :--- | :--- | :--- | :--- |
| file_name | VARCHAR(64) | PK | 落盘文件名，格式 **`{UUID v7}{.ext}`**（如 `0195e8b4-7c2a-7b3d-8c4d-1a2b3c4d5e6f.jpg`）；长度上限覆盖当前生成规则（约 41 字符） |
| artifact_id | VARCHAR(36) | NOT NULL, FK -> artifact.object_id | 所属文物主键 |
...
- `CONSTRAINT fk_artifact_image_artifact FOREIGN KEY (artifact_id) REFERENCES artifact(object_id) ON DELETE CASCADE`
- **`artifact` 软删除**（`is_deleted=1`）**不**级联删除 `artifact_image` 行；浏览接口在文物已软删时返回 404，不读图片表。
- **`artifact` 物理删除**时级联删除关联图片索引行；落盘文件清理由应用或运维脚本处理。

**与文件系统约定**：

- 二进制文件目录：`relics-images/`（可由 `app.relic-images.directory` 配置，默认开发环境为 `classpath:relics-images` 对应目录）。
- 公网访问路径：`{image-public-base-url}/relics-images/{file_name}`（由 `RelicImageResourceConfig` 映射静态资源）。
- 初始化脚本：`docker/mysql/artifact_image.sql`（须在 `artifact` 表创建之后执行）。
...
#### 表 `comment_like` / `artifact_like` / `user_favorite` / `user_browse_history`（互动扩展）

> 用于评论点赞、文物点赞、收藏、浏览历史等互动能力，也是文物列表 `popularity` 计算（浏览量 + 收藏量）的数据来源。

- `comment_like`：评论点赞关系，唯一键 `uk_comment_user(comment_id, user_id)`。
- `artifact_like`：文物点赞关系，唯一键 `uk_artifact_user(artifact_id, user_id)`。
- `user_favorite`：用户收藏关系（支持 `group_name` 分组），唯一键 `uk_user_artifact(user_id, artifact_id)`。
- `user_browse_history`：用户浏览历史，按 `browse_time` 记录行为时间。

**初始化与补丁脚本**：

- 标准初始化：`docker/mysql/interaction_tables.sql`
- 历史环境兜底补丁：`docker/mysql/04-interaction-tables-hotfix.sql`（幂等，可重复执行）
...
- **索引策略**：除上述标注索引外，对经常作为查询条件的字段酌情加索引，如 `artifact` 表中的 `museum_id`、`type`、`period`；`artifact_image.artifact_id` 用于按文物批量查封面与详情多图。
- **存量库迁移**：自 v1.0 升级时执行 `ALTER TABLE artifact DROP COLUMN image_url, DROP COLUMN image_path`，并运行 `docker/mysql/artifact_image.sql`；历史 `{objectId}.ext` 落盘文件须单独迁移登记入 `artifact_image`（见 PRD V2 P2）。
- **缺表修复**：若环境缺失互动表（常见报错：`user_browse_history` / `user_favorite` 不存在），执行 `docker/mysql/04-interaction-tables-hotfix.sql` 补齐结构。
